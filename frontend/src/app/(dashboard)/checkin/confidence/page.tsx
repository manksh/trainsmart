...
type Step = 'overview' | 'intro' | 'level' | 'sources' | 'actions' | 'commitment' | 'complete'
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        const confidenceCheckIns = historyData.check_ins.filter(c => c.check_in_type === 'confidence')
        setHistory({ check_ins: confidenceCheckIns, total: confidenceCheckIns.length })
      } catch (err) {
      ...
      <div className="min-h-screen bg-gradient-to-b from-amber-50 to-white">
        <div className="max-w-lg mx-auto px-4 py-8">
          ...
          <div className="text-center mb-6">
            ...
            <h1 className="text-2xl font-bold text-gray-900 mb-2">Confidence Check-In</h1>
            <p className="text-gray-600">Track your confidence levels over time</p>
          </div>

          <button
            onClick={() => setStep('intro')}
            className="w-full bg-amber-600 text-white py-4 rounded-xl font-medium hover:bg-amber-700 transition-colors mb-6"
          >
            Start New Check-In
          </button>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4">
                ...
                for (let day = 1; day <= daysInMonth; day++) {
                  ...
                  cells.push(
                    <div
                      ...
                      title={checkIn ? `Confidence: ${checkIn.confidence_level}/7` : ''}
                    >
                      {checkIn ? '💪' : day}
                    </div>
                  )
            ...
            <div className="flex items-center justify-center gap-4 mt-4 pt-4 border-t border-gray-100">
              ...
              <div className="flex items-center gap-2 text-xs text-gray-500">
                <div className="w-4 h-4 rounded-full ring-2 ring-amber-500 ring-offset-1"></div>
                <span>Today</span>
              </div>
            </div>
          </div>
                    ...
                    >
                      <div className="flex items-center justify-between mb-1">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="font-medium text-gray-900">Confidence Check-in</span>
                        </div>
                      ...
                      <div className="text-sm text-gray-500">
                        Confidence level: {item.confidence_level}/7
                      </div>
                    </div>
                  ))}
                </div>
